...
<div class="container" style="margin-top:50px;">
	<div class="row">
			<button onclick="location.href='mydog_register_form.do'" class="btn btn-info btn-sm">강아지 등록</button>
		...
		<div class="col-lg-8">
			<div class="row" style="border:1px solid black; float:left;">
					<c:forEach items="${requestScope.dogList }" var="dog">
						<div class="form-group" id="dogImage">
							...
							<c:forEach items="${dog.dogImage }" var="dogImage">
								<img src="${initParam.rootPath }/dogImage/${dogImage.dogImage }" width="200px" class="img-circle"><br>
							</c:forEach>
						</div>
					...
					<div class="btns">
						<button onclick="location.href='select_dog_dogInfo_dogImage.do?dogId=${dog.dogId}'" class="btn btn-warning btn-sm">강아지 수정</button>
						<button onclick="location.href='delete_dog.do?dogId=${dog.dogId}'" class="btn btn-danger btn-sm">강아지 삭제</button>
					</div>
				</div>
			</c:forEach>
		<div class="col-lg-2"></div>
