...
$(document).ready(function(){
	$("#addDogImage").on("click",function(){
		$("#addDog").append("<div class='form-group'><label for='dogImageId'> 강아지 이미지 : </label><input type='file' name='dogImageList' id='dogImage' class='form-controller'/></div>");
	});	
});
</script>
<h3>강아지 등록하기</h3>
<form id="addDog" action="${initParam.rootPath }/dog/insertDog.do" method="post">
	...
	<div class="form-group">
		...
		<input type="checkbox" name="dogInfoList" id="dogInfo" value="${dogInfo }" class="form-controller"/>
			${dogInfo.codeName }
		</c:forEach><br>
	</div>
	<div class="form-group">
		<label for="dogImageId">강아지 이미지 : </label>
		<input type="file" name="dogImageList" id="dogImage" class="form-controller" />
	</div>
